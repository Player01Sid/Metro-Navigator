...
void addStation(struct MetroMap *map, char station[], int lineColor)
{
    int index = map->numStations++;
    map->graph[index] = createNode(station, lineColor);
}

void addConnection(struct MetroMap *map, char station1[], char station2[], int lineColor)
{
...
int main()
{
    ...
    dijkstra(metroMap, "Station A", "Station D");
